...
def parse_template() :
    ...
    for line in f :
        ...
        if "%ranking_ave_tempera_low%" in line :
            ...
            continue
        if "%ranking_max_tempera_low%" in line :
            tempera.ranking_max_tempera_low(out)
            continue
        if "%ranking_min_tempera_low%" in line :
            tempera.ranking_min_tempera_low(out)
            continue
        ...
        if "%weekly_tempera_table%" in line :
            ...
            continue
        if "%ranking_week_tempra_top%" in line :
            tempera.ranking_week_tempra_top(out)
            continue

        if "%ranking_daily_diff%" in line :
            tempera.ranking_daily_diff(out)
            continue
        if "%ranking_week_diff7_top%" in line :
            tempera.ranking_week_diff(out,7)
            continue
        if "%ranking_week_diff14_top%" in line :
            tempera.ranking_week_diff(out,14)
            continue
        if "%ranking_week_diff365_top%" in line :
            tempera.ranking_week_diff_year(out)
            continue
        if "%ranking_week_diff7_low%" in line :
            tempera.ranking_week_diff_low(out,7)
            continue
        if "%ranking_week_diff14_low%" in line :
            tempera.ranking_week_diff_low(out,14)
            continue
        ...
        if "%top_continuous_fine%" in line :
            ...
            continue
        if "%ranking_prec_daily%" in line :
            rain.ranking_prec_daily(out)
            continue
